...
const Wizard = React.memo(function Wizard(props) {
  ...
  useEffect(() => {
    // Ensure the model is loaded before applying animations
    if (modelRef.current) {
      modelRef.current.position.set(0, -1.5, 0);
      modelRef.current.scale.set(0.06, 0.06, 0.06);
      modelRef.current.rotation.set(0.25, 0, 0);
    }
    // Cleanup function to reset the model when unmounted
    return () => {
      if (modelRef.current) {
